...
public class S3ContentReader extends AbstractContentReader implements AutoCloseable {
  ...
  protected void closeFileObject() throws IOException {
    if (s3Object != null) {
      if (LOG.isTraceEnabled()) {
        LOG.trace("\n\n -------------- Closing s3 file object for reader " + key);
      }
      s3Object.close();
      s3Object = null;
    }
  ...
  protected void lazyInitFileObject() {
    if (s3Object == null) {
      if (LOG.isTraceEnabled()) {
        LOG.trace("\n\n -------------- Lazy init for file object for " + bucket + " - " + key);
      }
  ...
  protected void lazyInitFileMetadata() {
    if (s3ObjectMetadata == null) {
      if (LOG.isTraceEnabled()) {
        LOG.trace("\n\n -------------- Lazy init for file metadata for " + bucket + " - " + key);
      }
      ...
      if (s3Object != null) {
        ...
        if (LOG.isDebugEnabled()) {
          LOG.debug("\n\n -------------- GETTING OBJECT METADATA - BUCKET: " + bucket + " KEY: " + key);
        }
  ...
  protected ContentReader createReader() throws ContentIOException {
    if (LOG.isDebugEnabled()) {
      LOG.debug("\n\n -------------- Called createReader for contentUrl -> " + getContentUrl() + ", Key: " + key);
    }
    ...
    try {
      ...
      ContentStreamListener s3StreamListener = new ContentStreamListener() {
        ...
        public void contentStreamClosed() throws ContentIOException {
          try {
            LOG.trace("\n\n -------------- Closing s3 object stream on content stream closed.");
            closeFileObject();
          } catch (IOException e) {
  ...
  private S3Object getObject() {
    ...
    try {
      if (LOG.isDebugEnabled()) {
        LOG.debug("\n\n -------------- GETTING OBJECT - BUCKET: " + bucket + " KEY: " + key);
      }
